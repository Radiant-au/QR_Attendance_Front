...
import { Home, QrCode, LogOut, LayoutDashboard, Users, Calendar, Menu, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row">
      ...
      <div className="flex-1 flex flex-col md:pl-64">
        ...
        {role === Role.USER && (
          <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-200 px-6 py-3 flex justify-around items-center md:hidden z-50">
            {userNav.map((item) => (
              <button
                key={item.path}
                onClick={() => navigate(item.path)}
